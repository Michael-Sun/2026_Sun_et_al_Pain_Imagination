function [cm, colorbar1_han, colorbar2_han] = plot_to_surf(surf_vert_val, surface_handles, varargin)
...
%
%   **surf_func:**
%       - surface vertex values. Must match number of vertices in surface
%       handles
%
%   **han:**
...
if ~isempty(pos_colormap) ||  ~isempty(neg_colormap)
    ...
    if doindexmap
        ...
        nvals = min([256, length(unique(surf_vert_val))]);
        cm = colormapname;
...
if any(datvec < 0) && has_colorbar
    ...
    if doindexmap
        ...
        maxneg = max([max(datvec(datvec < 0)),min([0,clim(2)])]);
...
function [datvec, clim] = get_data_range(datavec, clim)

wh = datavec ~= 0 & ~isnan(datavec);
dat = datavec(wh);
...
function out = map_function(x, a, b, A, B)
    if b == a
        out = ones(size(x)) * round((A + B)/2);  % avoid division by zero
    else
        out = A + (x - a) ./ (b - a) * (B - A);
    end
    out = max(min(out, B), A);  % clamp
end